...
namespace joyhuang\Push\MzPushSDK;
...
class MzPush extends MzPushBase
{
     ...
     * 应用标签推送
     * @param $pushType 取值为1或者0,1代办透传消息，0:代表通知栏
     * @param $tagNames  推送标签 必填 多个通过英文逗号分割
     * @param $message  对应的消息对象
     *
     * @return int|mixed
     */
    public function pushToTags( $pushType, $tagNames, $message )
    {
        if ( $pushType == STATUSBAR ) {
            if ( !$message instanceof VarnishedMessage ) {
                return ERROR_NOT_VARNISHED_MESSAGE;
        ...
        $this->params = array(
            'pushType'    => $pushType,
            'tagNames'    => is_array($tagNames) ? implode(',', $tagNames) : $tagNames,
            'scope'       => 0,
            'messageJson' => $message->build()
        );
        return $this->post(
            $this->host . '/garcia/api/server/push/pushTask/pushToTag'
        );
    }
